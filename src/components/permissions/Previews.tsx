...
// Example scenes, one per permission. Each shows only that permission's use
// cases, so the examples read as "this is what it's for".
...
/** The three kinds of notifications: a match, a message, an event. */
export function NotificationsHero() {
  ...
  return (
    <div className="w-full space-y-[10px]">
      {banners.map((b, i) => (
        <motion.div key={b.title} className="flex gap-3 rounded-[22px] border border-white/10 bg-white/[0.08] p-3" {...enter(0.1 + i * 0.15)}>
          <AppIcon size={38} />
...
export function VoiceHero() {
  return (
    <div className="flex w-full flex-col items-center">
      <motion.div {...enter(0)}>
        <MuseAvatar muse={DEFAULT_MUSE} size={72} mood="listening" />
      </motion.div>
      <motion.div className="mt-4" {...enter(0.15)}>
        <Waveform bars={26} height={22} />
      </motion.div>
      <div className="mt-5 w-full space-y-[10px]">
...
function Bubble({ label, delay, children }: { label: string; delay: number; children: ReactNode }) {
  return (
    <motion.div className="rounded-[18px] border border-white/10 bg-white/[0.06] px-4 py-[10px]" {...enter(delay)}>
      <div className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[#FF8AA2]">{label}</div>
      <p className="mt-[2px] text-[14px] leading-[20px] text-white/85">{children}</p>
    </motion.div>
  );
}
      ...
      <motion.div className="flex flex-col items-center" {...enter(0)}>
        <div className="relative h-[176px] w-full">
          {fan.map((s, i) => (
            <motion.div
              key={s.id}
              className="absolute top-2 h-[150px] w-[88px] overflow-hidden rounded-[14px] border-2 border-[#1C1B22] shadow-xl"
              style={{ left: `calc(50% - 44px + ${(i - 1) * 22}px)`, zIndex: i === 1 ? 2 : 1 }}
        ...
        <div className="mt-1 text-center text-[14px] font-semibold">Add your moments</div>
      </motion.div>

      <motion.div className="flex flex-col items-center" {...enter(0.2)}>
        <div className="relative mt-2 h-[160px] w-[114px] overflow-hidden rounded-[18px]">
          {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        <div className="mt-[13px] text-center text-[14px] font-semibold">Verify it&apos;s you</div>
      </motion.div>
    </div>
  );
}
...
export function ShortcutHero() {
  return (
    <div className="flex w-full flex-col items-center">
      <motion.div {...enter(0)}>
        <ActionButtonArt size={1.8} />
      </motion.div>
      <motion.div className="mt-7 flex items-center gap-3 rounded-full border border-white/10 bg-white/[0.08] py-2 pl-2 pr-5" {...enter(0.25)}>
